...
<div class="page-content">
    <form id="roleAndPermissionsForm" class="form-inline" action="saveRoleAndPermissions" method="post">
    	<div class="panel panel-default">
    		...
    		<div class="panel-body">
    			<div class="form-group">
                     ...
                     <button type="button" onclick="IsNull()" class="btn btn-primary">保存角色和权限</button>
                </div>
    		</div>
    	</div>
        <div class="panel panel-default">
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<table class="table table-bordered table-striped" width="800px">
						...
						<tr>
						    <td>链接</td>
						    <td>
								<input class="form-control" name="url" placeholder="链接">
								<p id="p2"></p>
						    </td>
						</tr>
						<tr>
				...
				<div class="modal-footer">
					<button type="button" class="btn btn-success" <%--data-dismiss="modal"--%>
						aria-hidden="true" onclick="validateForm()">保存</button>
...
<script type="text/javascript">
	function IsNull(){
		...
		if(roleName.length==0){
			alert('对不起，角色名不能为空或者为空格!');
		}else {
			$.ajax({
				url:"saveRoleAndPermissions",
				type: "post",
				data: $("#roleAndPermissionsForm").serialize(),
				error: function (message) {
					alert("保存失败");
				},
				success: function (message) {
					alert(message.srapMSG);
					location.reload();
				}
			});
		}
	...
	function saveSubmitPermission() {
		$.ajax({
			url:"saveSubmitPermission",
			type: "post",
	...
	function validateForm() {
		...
		$("#p2").empty();
		$("#p3").empty();
		var name = document.forms["permissionForm"]["name"].value;
		var url = document.forms["permissionForm"]["url"].value;
		var percode = document.forms["permissionForm"]["percode"].value;
		if (isNull(name)) {
			$("#p1").html("权限名称不能为空！").css('color', 'red');
		}
		if (isNull(url)) {
			$("#p2").html("链接不能为空！").css('color', 'red');
		}
		if (isNull(percode)) {
			$("#p3").html("标识符不能为空！").css('color', 'red');
			return false;
		}
		this.saveSubmitPermission();
	}
